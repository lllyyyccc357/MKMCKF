function [out,qtho] = Thomas_eliminateMag_EKF(acc, gyr, mag, t, stdAcc, stdGyro, stdMag, sigma_acc,sigma_mag)
...
q_s_acc=QuaternionsProd(q_s2e_gyr,QuaternionsProd([0;0;g;0],q_e2s_gyr));
r_s_acc=q_s_acc(1:3,:);
a=[ax(i);ay(i);az(i)];
alpha_err_acc=acos(dot(a,r_s_acc)/norm(a)/norm(r_s_acc));
x_corr_acc=cross(a,r_s_acc)/norm(cross(a,r_s_acc));
% q_as_acc=[sin(1/2*alpha_err_acc)*x_corr_acc;cos(1/2*alpha_err_acc)];


k_acc=0;
q_corr_acc=[sin(1/2*k_acc*alpha_err_acc).*x_corr_acc;cos(1/2*k_acc*alpha_err_acc)];
q_e2s_gyracc=QuaternionsProd(q_e2s_gyr,q_corr_acc);
q_e2s_gyracc=q_e2s_gyracc/QuaternionsNorm(q_e2s_gyracc);
% xPriori(:,i)=q_e2s_gyracc;
m=[hx(i);hy(i);hz(i)];
m_hat_s=m-dot(m,r_s_acc)*r_s_acc;

q_s_mag=QuaternionsProd(q_s2e_gyr,QuaternionsProd([h(1);0;0;0],q_e2s_gyr));
r_s_mag=q_s_mag(1:3,:);
alpha_err_mag=acos(dot(m,r_s_mag)/norm(m)/norm(r_s_mag));
x_corr_mag=cross(m_hat_s,r_s_acc)/norm(cross(m_hat_s,r_s_acc));


k_mag=0;
q_corr_mag=[sin(1/2*k_mag*alpha_err_mag).*x_corr_acc;cos(1/2*k_mag*alpha_err_mag)];
q_e2s_gyraccmag=QuaternionsProd(q_e2s_gyracc,q_corr_mag);
q_e2s_gyraccmag=q_e2s_gyraccmag/QuaternionsNorm(q_e2s_gyraccmag);
xPriori(:,i)=q_e2s_gyraccmag;
h(3)=0;
    %% this is MKMC part
    z=[a;m_hat_s]; %measurement vector

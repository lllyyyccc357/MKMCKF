function [out,qsab] = SAB(acc, gyr, mag, t, stdAcc, stdGyro, stdMag, thAcc, thMag, stdBiasMag_in)
...
for i=1:length(t)-1
  ...
  %      zeros(3,4) eye(3)]; %linearized to increase computational speed
    ...
    xPost(5:7,i+1)=zeros(3,1);
    %Compute the a posteriori covariance matrix
    Ppost=Ppriori-K*H*Ppriori;

    % store
    erra=[ax(i);ay(i);az(i)]-C*[0;0;g];
    errh=([hx(i);hy(i);hz(i)]-xPriori(5:end,i)-C*h);
    Erra(i,:)=erra';
    Errh(i,:)=errh';
end

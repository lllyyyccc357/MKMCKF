function [out,qsab] = SAB_New(acc, gyr, mag, t, stdAcc, stdGyro, stdMag, thAcc, thMag, stdBiasMag_in)
...
for i=1:length(t)-1
  ...
  %      zeros(3,4) eye(3)]; %linearized to increase computational speed
    ...
    %UPDATE STEP
    q_last=[xPriori(4,i),xPriori(1,i),xPriori(2,i),xPriori(3,i)];
    q_last=quaternion(q_last);
    mq=[0,mag(i,:)];
    mq=quaternion(mq);
    ...
    %Magnetometer
    errh=norm([hx(i);hy(i);hz(i)]-C*h);
    if norm([hx(i);hy(i);hz(i)]-C*h)<thMag
        std_mag=stdMag;
    else
        std_mag=1e100;
    end

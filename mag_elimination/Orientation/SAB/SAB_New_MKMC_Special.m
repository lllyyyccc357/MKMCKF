function [out,qsab] = SAB_New_MKMC_Special(acc, gyr, mag, t, stdAcc, stdGyro, stdMag, sigma_acc,sigma_mag,reset_index)
...
warning off
for i=1:length(t)-1
    %if(i==10600||i==21200||i==32000)
    for initm=1:length(reset_index)
    if(i==reset_index(initm))
        %% init
        accr=-acc(i,:); % gravity neagtive 
        magr=mag(i,:);
        % NED coordinate
        r_down=accr';
        r_east=cross(accr',magr');
